...
Currently configured project: `learn-tanstack-start` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/learn-tanstack-start/SETUP.md` to fill in
   `data/learn-tanstack-start/INFO.md`.
...
`--project-id learn-tanstack-start` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/learn-tanstack-start/`.
