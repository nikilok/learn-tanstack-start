...
/**
 * Brave brand logo — the lion mark on the orange-to-red shield. `aria-hidden`;
 * sized via `className`. Gradient IDs are scoped with `useId` so two instances on
 * one page can't clash.
 */
export default function BraveIcon({ className }: { className?: string }) {
  const id = useId();
  return (
    <svg
    ...
    >
      <linearGradient id={`${id}-a`} y1="51%" y2="51%">
        <stop offset=".4" stopColor="#f50" />
        <stop offset=".6" stopColor="#ff2000" />
      </linearGradient>
      <linearGradient id={`${id}-b`} x1="2%" y1="51%" y2="51%">
