...
/** Leaflet map centered on `geo` with a custom Union-Jack-pin marker (teardrop body + the shared `UnionJackLens` overlaid as the head). Scaffolding (tiles, theme, zoom clamps) comes from `MapShell`. Loaded client-side only via the `AddressMap` lazy import — Leaflet touches `window`/`document` at import time and can't run on the SSR server. */
export default function LeafletMap({
  ...
  const markerTitle = companyName
    ...
    : 'Company location';
  return (
    <MapShell center={position} zoom={16}>
