...
-- IF NOT EXISTS mirrors the 0039 twin: an environment whose indexes already
-- exist (restored snapshot, or 0039 applied first) must not abort the run.
CREATE INDEX IF NOT EXISTS "idx_mapping_org_slugified" ON "hmrc_company_mapping" USING btree ((btrim(regexp_replace(lower("organisation_name"), '[^a-z0-9]+', '-', 'g'), '-')));--> statement-breakpoint
